...
- [x] AI bag entry — three independent inputs: bag photo, product URL (fetched server-side), and free-text notes; any combination works; Claude extracts and merges structured fields from all provided sources; graceful URL fetch fallback (shows warning, continues with remaining sources); strict anti-hallucination — never infers region/farm/variety from training knowledge; after parsing, DB lookup backfills missing coffee-level fields (roast level, process, origins) from any prior bag of same roaster+name; dial-in tip uses shot history from previous bags of same coffee as primary grind reference
