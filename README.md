...
- [x] Per-bag shot analysis charts — Grind vs Taste scatter, Rating Trend (true bag-scoped shot number on X, not a filtered index — unrated shots leave a visible gap), Extraction Ratio bar, Yield vs. Time scatter with target-ratio reference zones at the bag's average dose (dot color: green on-target, orange off-target, gray no-target, red failed) (server-side SVG)
...
- [x] AI analysis reliability — `max_tokens` raised from 3000 to 8000 (a truncated response produces invalid JSON that used to get stored verbatim as a garbled "summary"); `stop_reason === "max_tokens"` is detected and discarded before parsing instead of persisting a broken record; any JSON parse failure now returns null (retriable via the existing "Analyze with AI" / "Re-analyze" button) instead of silently "succeeding" with garbage
- [x] `known_patterns` auto-compression — once the AI coach's accumulated notes exceed 2500 characters, a small dedicated call condenses them toward ~1500 characters (preserving specific actionable facts, dropping stale/superseded entries) instead of growing unbounded and eventually re-triggering the truncation above; fails open (keeps the original text) if the compression call itself fails
